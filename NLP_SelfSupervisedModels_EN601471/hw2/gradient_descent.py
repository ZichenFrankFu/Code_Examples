...
def featurize(sentence: str, embeddings: gensim.models.keyedvectors.KeyedVectors) -> Union[None, torch.FloatTensor]:
    ...
    if len(vectors) == 0:
        ...
        avg_embed = torch.zeros(embeddings.vector_size)
        for vec in vectors: 
            avg_embed += torch.FloatTensor(vec)
        avg_embed /= len(vectors)
    return avg_embed
...
class SentimentClassifier(nn.Module):
    ...
    def softmax(logits):
        ...
        const_adjust, _ = torch.max(logits, dim=1, keepdim=True)
        logits = logits - const_adjust
        exp_vals = torch.exp(logits)
        exp_sum = torch.sum(exp_vals, dim=1, keepdim=True)
    ...
    def gradient_loss(self, inp, logits, labels):
        ...
        one_hot_label = nn.functional.one_hot(labels).float()
        pred = self.softmax(logits)
        epsilon = 1e-7
        loss = 0
        loss_vec = torch.sub(pred, one_hot_label)

        # loss
        for n in range(bsz): #batch_size
            dot = torch.dot(torch.log(pred[n][:]), one_hot_label[n][:])
            loss += dot

        loss *= -(1/(bsz + epsilon)) #num_class
 
        # matrix of gradient about weights
        grads_weights = torch.t(torch.mul(1/(bsz), 
                                          torch.matmul(torch.transpose(inp, 0, 1), loss_vec)))

        # matrix of gradient about bias
        grads_bias = torch.zeros(self.num_classes,)
        for cl in range(self.num_classes):
            grads_bias = torch.add(grads_bias, loss_vec[cl])
        grads_bias = torch.mul(1/(bsz), grads_bias)
        # your code ends here
...
def accuracy(logits: torch.FloatTensor , labels: torch.LongTensor) -> torch.FloatTensor:
    ...
    correct = torch.eq(pred_label, labels)
    return torch.Tensor.int(correct)
