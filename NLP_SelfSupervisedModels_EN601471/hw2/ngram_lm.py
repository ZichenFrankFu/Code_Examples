...
def create_ngrams(data, n, splitter, tokenizer):
    ...
    for paragraph in tqdm(data['text']):
        ...
        for sentence in splitter.split(paragraph):
            ...
            for idx, _ in enumerate(tokens):
                ...
                curr_ngram = tuple(tokens[idx:idx+n])
                context = curr_ngram[:-1]
                potential_next = curr_ngram[-1]
                ngram_context[context] += 1
                ngrams[curr_ngram] += 1
                next_word_candidates[context].add(potential_next)
    ...
    for context, next_words in sorted_next_word_candidates.items():
        ...
        for nw in next_words:
            ...
            ngram_count = ngrams[context + (nw,)]
            context_count = ngram_context[context]
            if context_count == 0:
                # avoiding divide by 0
                scores.append(0)
            else:
                scores.append(ngram_count / context_count)
            # Your code ends here
...
def plot_next_word_prob(word_scores, word_candidates, context, top=10, save_path=None):
    ...
    sorted_probs_words = sorted(zip(word_scores[context],word_candidates[context]), reverse=True)[0:top]
    probs,words = zip(*sorted_probs_words)
    
    # Figure Size
    fig = plt.figure(figsize =(16, 9))
    plt.bar(words, probs, color ='maroon', width = 0.4)
    plt.xlabel("Next words")
    plt.ylabel("Probability")
    plt.title("TOP " + str(top) + " potential next words for the given context " + str(context))
    if (save_path):
