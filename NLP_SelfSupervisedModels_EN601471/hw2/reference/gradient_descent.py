...
def featurize(sentence: str, embeddings: gensim.models.keyedvectors.KeyedVectors) -> Union[None, torch.FloatTensor]:
    ...
    return torch.from_numpy(avg_vector)
...
class SentimentClassifier(nn.Module):
    ...
    def softmax(logits):
        ...
        offset, _ = torch.max(logits, dim=1, keepdim=True)
        logits = logits - offset
        exp_vals = torch.exp(logits)
        norm = torch.sum(exp_vals, dim=1, keepdim=True)
    ...
    def gradient_loss(self, inp, logits, labels):
        ...
        # print(f"shape labels: {labels.size()}")
        # print(labels)
        # print(probs)
        grads_weights = torch.mm(inp.t(), probs - labels) / bsz
        grads_bias = torch.sum((probs - labels),dim=0) / bsz
        log_probs = torch.log(probs + 1e-6)
        loss = - torch.sum(torch.mul(labels, log_probs)) / bsz
        # print(grads_weights)
        # print(grads_bias)
        # print(loss)
        # your code ends here
        return grads_weights.transpose(0,1), grads_bias, loss
