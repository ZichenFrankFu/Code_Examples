...
def create_ngrams(data, n, splitter, tokenizer):
    ...
    for paragraph in tqdm(data['text']):
        ...
        for sentence in splitter.split(paragraph):
            ...
            for idx, _ in enumerate(tokens):
                ...
                ngrams[ngram] += 1
                next_word_candidates[context].add(next)
    ...
    for context, next_words in sorted_next_word_candidates.items():
        ...
        for nw in next_words:
            ...
            numerator = ngrams[ngram]
            denominator = ngram_context[context]
            if denominator == 0:
                scores.append(0)
            else:
...
def plot_next_word_prob(word_scores, word_candidates, context, top=10, save_path=None):
    ...
    sorted_tuples = sorted(zip(word_scores[context],word_candidates[context]), reverse=True)[0:top]
    sorted_scores,sorted_candidates = zip(*sorted_tuples)
    fig = plt.figure(figsize = (10, 5))
    plt.bar(sorted_candidates, sorted_scores, color ='maroon', 
            width = 0.4)
    plt.xlabel("Candidate words")
    plt.ylabel("Scores")
    plt.title("Top next word probabilities given " + str(context))
    if (save_path):
