...
def explore_mlp_activations(dev_d: Dict[str, List[Union[str, int]]],
    ...
    for ac, ac_name in zip(activations, activation_names):
        train_config = EasyDict({
            'batch_size': batch_size,  # we use batching for training
            'lr': lr,
            'num_epochs': num_epochs,  # the total number of times all the training data is iterated over
            'hidden_dims': hidden_dims,  # the number of neurons in each hidden layer
            'save_path': f'model_activation_{ac_name}.pth',  # path where to save the model
            'embeddings': embedding_type,
            'num_classes': num_classes,
            'activation': str(ac),  # non-linear activation function
        })
        ...
        all_emb_epoch_dev_accs.append(epoch_dev_accs)
        all_emb_epoch_dev_losses.append(epoch_dev_loss)
        visualize_epochs(epoch_train_losses, epoch_dev_loss, f"mlp_activation_{ac_name}_loss.png")
...
def explore_mlp_learning_rates(dev_d: Dict[str, List[Union[str, int]]],
    ...
    lrs = [0.0025, 0.01, 0.02, 0.05, 0.1]
    lrs_names = ['0_0025', '0_01', '0_02', '0_05', '0_1'] # for visualization
    ...
    for lr, lr_name in zip(lrs, lrs_names):
        train_config = EasyDict({
            ...
            'num_epochs': num_epochs,  # the total number of times all the training data is iterated over
            'hidden_dims': hidden_dims,  # the number of neurons in each hidden layer
            'save_path': f'model_learningRate_{lr_name}.pth',  # path where to save the model
            'embeddings': embedding_type,
            'num_classes': num_classes,
            'activation': activation,  # non-linear activation function
        })
        ...
        all_emb_epoch_dev_accs.append(epoch_dev_accs)
        all_emb_epoch_dev_losses.append(epoch_dev_loss)
        visualize_epochs(epoch_train_losses, epoch_dev_loss, f"mlp_learningRates_{lr_name}_loss.png")

    visualize_configs(all_emb_epoch_dev_accs, lrs_names, "Accuracy", "./all_mlp_learningRates_acc.png")
    visualize_configs(all_emb_epoch_dev_losses, lrs_names, "Loss", "./all_mlp_learningRates_loss.png")
    # your code ends here
