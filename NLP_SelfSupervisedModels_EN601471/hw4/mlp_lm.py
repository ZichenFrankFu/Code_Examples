...
class NPLMFirstBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        embedded = embedded.view(inputs.shape[0], -1)

        # Transform concatenated embeddings with a linear layer and tanh activation
        transformed = torch.tanh(self.linear(embedded))
...
class NPLMFinalBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        log_probs = torch.log_softmax(transformed, dim=-1)
...
class NPLM(nn.Module):
    ...
    def forward(self, inputs):
        # TODO: implement the forward pass
        # input layer
        log_probs = nn.functional.relu(self.first_layer(inputs))

        # multiple middle layers
        # remember to apply the ReLU activation function after each layer
        
        if len(self.intermediate_layers) != 0: 
            for blocks in self.intermediate_layers:
                log_probs = nn.functional.relu(blocks(log_probs))

        # output layer
        log_probs = self.final_layer(log_probs)
